...
namespace proyecto_chicha_deleali
{
    public partial class FormRecuperar : Form
    {
        ...
        {
            Form datagreviu = new datagreviu_eliminados();
            datagreviu.Show();
        }
        ...
        private void boton_recuperar_Click(object sender, EventArgs e)
        {
            conn.Open();
            ...
            if (txt_buscar_id.Text != "")
            {
                if (existe_bd == 0)
                {
                    ...
                    using (SQLiteCommand cmdUpdate = new SQLiteCommand($"UPDATE productos SET existe = @existe WHERE ID_Productos = @ID_Productos", conn))
                    {
                        ...
                        {
                            MessageBox.Show("Se ha Recuperado el producto con éxito.", "Éxito", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                        {
                ...
                {
                    MessageBox.Show("Este producto ya ha sido recuperado.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    conn.Close();
                }
            }
            else
